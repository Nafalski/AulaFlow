...
- Históricos de aulas em `/professor/historico` e `/aluno/historico`: conclusão, cancelamento, reagendamento e falta/no-show ficam consultáveis em páginas limitadas; RSVP, presença e efeito de crédito permanecem conceitos separados na visão do aluno
...
- Listas de crescimento contínuo usam paginação no servidor com ordem estável: alunos, turmas, pacotes atribuídos, pacotes e turmas na ficha do aluno, históricos de pacotes/aulas e avisos; filtros permanecem no URL e não há contagens totais pesadas
...
`/professor/clubes` gere contextos, clubes e membros; `/professor/clubes/[id]/calendario` mostra a disponibilidade partilhada do clube; `/professor/convites` mostra os convites recebidos. `/professor/pacotes` gere modelos reutilizáveis, atribuição, consulta e ajustes administrativos dos pacotes atribuídos. `/professor/pacotes/historico` mostra a auditoria global. `/professor/definicoes/disponibilidade` guarda a fonte de verdade da agenda do professor. `/professor/calendario` e `/aluno/calendario` mostram disponibilidade e aulas em Dia/Semana/Mês; `/professor/historico` e `/aluno/historico` mostram os desfechos passados. `/aluno/pacotes` mostra apenas os próprios pacotes e movimentos básicos.

A **Etapa 8A** ligou a fundação de notificações que existia desde a Fase 1: marcar, reagendar ou cancelar uma aula deixa um aviso na caixa do aluno, com o horário do momento em que aconteceu. Os avisos marcam-se como lidos e o sino mostra quantos faltam. A **Etapa 8B** acrescentou o agendador: um trabalho no próprio PostgreSQL corre de hora a hora e deixa na caixa um lembrete até 24 horas antes da aula e outro até duas horas antes, avisa quando um pacote está a acabar ou a expirar, e dá por expirado o pacote cuja validade passou — sem mexer em créditos nenhuns. O lembrete mais cedo não promete "amanhã": a janela apanha também aulas do próprio dia, e o aviso diz a data e a hora reais. A **Etapa 8C** liga a entrega por email: cada aviso deixa também um pedido de envio numa fila, e um trabalho separado trata de o enviar — com preferências por tipo de aviso e horas de silêncio no fuso da própria conta. Cancelar ou marcar uma aula nunca espera pelo servidor de email, e continua a funcionar se ele estiver em baixo. A **Etapa 8D** reviu o sistema como uma cadeia única e fechou a Fase 8 sem alterações de runtime: 601 testes, 1.113 verificações PostgreSQL locais, 115 verificações de catálogo remoto, duas passagens consecutivas de 666 verificações Auth/PostgREST e 245 verificações de browser tanto em DEV como no build de produção. A Edge Function, o Resend e os ciclos automáticos de `pg_cron`/`pg_net` permanecem validados de ponta a ponta em DEV; durante a revisão não foi enviado email a nenhum utilizador real do AulaFlow. **Push e WhatsApp continuam fora do MVP, e a Fase 9/preparação de produção é separada.** O professor independente continua totalmente suportado, com workspace pessoal privado e agenda própria, e não precisa criar clube nenhum.
...
Ainda não existe política configurável de cancelamento, janela de 12h/24h, cancelamento self-service do aluno, reativação de participação cancelada nem pagamentos.

Sem um bucket de Storage configurado, os avatares usam iniciais. Preparar a ligação de um aluno continua sem enviar email por si; os avisos produzidos pelas operações e pelo agendador passam pelo outbox da 8C, que respeita as preferências e as horas de silêncio antes de qualquer entrega externa.
