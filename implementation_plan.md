...
O Vault de DEV conserva `aulaflow_email_worker_url` e `aulaflow_email_worker_token`. Os dois jobs permanecem únicos e ativos, e ciclos automáticos consecutivos do worker chegaram à função real com HTTP `200` e `claimed = 0`. A ativação fica permanente em DEV.

**Hardening transversal posterior ao fecho da 8C:** o catálogo real revelou 13 funções internas ainda com o `EXECUTE` que o PostgreSQL concede a `PUBLIC` por omissão. Onze são funções de trigger e duas são helpers do pipeline de notificações/agendamento. Uma migração incremental fechou explicitamente `PUBLIC`, `anon` e `authenticated` nessas assinaturas, sem alterar corpos, RLS, cron, worker ou comportamento de domínio. É defesa em profundidade da fronteira de RPC; a 8D confirmou que a fronteira continua fechada e que as RPCs legítimas e os triggers continuam operacionais.

#### Fechamento integrado da Fase 8 — Etapa 8D

A revisão integrada percorreu `evento de domínio -> notificação -> caixa in-app -> preferência -> outbox -> silêncio/claim/lease -> pg_cron/pg_net -> Edge Function -> fornecedor`. Não encontrou regressão funcional ou de segurança e, por isso, não criou migração nem alterou o runtime. Os cenários de criação, reagendamento, cancelamento, lembretes e avisos de pacote preservaram transação, snapshot histórico, isolamento, deduplicação e ausência de acoplamento ao fornecedor.

O fechamento passou com 601 testes Vitest, 1.113 verificações PostgreSQL locais, 115 verificações remotas de catálogo, 666 verificações Auth/PostgREST em duas execuções consecutivas e 245 verificações de browser tanto no servidor DEV como no build de produção. A Edge Function permaneceu `ACTIVE`; Vault, os dois cron jobs e ciclos `pg_net` com HTTP `200` foram confirmados sem expor segredos. As seis contas E2E terminaram com email externo desligado e avisos internos ativos; nenhuma entrega para `@aulaflow.test` ficou enviável e nenhum utilizador real do AulaFlow recebeu email durante a revisão.

**Fase 8A fechada. Fase 8B fechada. Fase 8C fechada. Fase 8D fechada. Fase 8 fechada em DEV.** Push e WhatsApp continuam fora do MVP; a Fase 9 e a preparação de produção permanecem trabalho separado.
