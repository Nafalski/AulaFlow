-- ============================================================================
-- AulaFlow — 51. Fase 6, Etapa 6C.2: uma participação libertada continua libertada
-- ============================================================================
--
-- Ao copiar as participações não reservadas para a substituta,
-- `reschedule_lesson()` colapsava tudo o que não fosse `exempt` em `pending`.
--
-- Para uma participação cancelada isso é falso: o crédito já foi devolvido por
-- `release_participation_credits()` e não volta a ser cobrado. `pending` diz
-- "por cobrar" — e é o que a conclusão da aula e a área do professor leem.
--
-- O reagendamento move a aula; não reabre a cobrança de quem já desistiu.
-- ============================================================================
  ...
  loop
    if v_participant.billing_status = 'reserved' then
      ...
      values (
        ...
        -- Uma participação já libertada continua libertada na substituta.
        -- Chamar-lhe `pending` diria que ainda está por cobrar, quando o
        -- crédito já voltou ao aluno e não vai ser cobrado nunca.
        (case
