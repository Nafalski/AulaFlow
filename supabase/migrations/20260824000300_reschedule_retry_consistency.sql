-- ============================================================================
-- AulaFlow - consistencia de retries e RSVP no reagendamento
-- ============================================================================
--
-- Duas chamadas concorrentes da mesma intencao conseguiam observar a chave
-- ausente antes de uma delas bloquear a aula original. A vencedora concluia o
-- reagendamento; a segunda acordava depois do commit e tratava a original ja
-- historica como erro, apesar de ser apenas um retry equivalente.
--
-- O lock transacional abaixo serializa ator + chave ANTES da consulta da
-- intencao. Quem chega depois reconsulta o estado ja confirmado, valida a
-- intencao inteira e devolve a mesma substituta. Chaves diferentes continuam a
-- serializar pela linha da aula original e uma intencao diferente continua a
-- ser conflito.
--
-- O ramo de participantes sem reserva copiava o RSVP mas omitia confirmed_at,
-- tornando representavel status='confirmed' com o carimbo historico perdido.
-- A resposta original passa a viajar em ambos os ramos do reagendamento.
-- ============================================================================
...
begin
  ...
  -- Namespace distinto dos locks de conflito e de criacao. Colisoes do hash
  -- apenas serializam duas intencoes independentes; nunca alteram o resultado.
  perform pg_advisory_xact_lock(
    17051004,
    hashtext(v_actor::text || ':' || p_idempotency_key::text)
  );

  -- Esta leitura acontece DEPOIS do lock de intencao. Num retry concorrente,
  -- portanto, ja ve o commit do vencedor e devolve a mesma substituta.
  ...
  values (
    ...
    v_original.notes_for_students, v_original.private_notes, v_original.status,
  ...
  loop
    if v_participant.billing_status = 'reserved' then
      ...
      values (
        ...
        v_participant.confirmed_at, v_participant.declined_at, v_participant.decline_reason,
        v_participant.student_package_id, 0, 0,
        (case
           when v_participant.billing_status = 'exempt' then 'exempt'
           when v_participant.billing_status = 'released' then 'released'
           else 'pending'
...
comment on function public.reschedule_lesson(
  ...
  'Único caminho para reagendar. Serializa ator + chave antes de consultar a '
  'intenção: retries concorrentes equivalentes devolvem a mesma substituta, e '
  'intenções diferentes continuam em conflito. Preserva RSVP e o confirmed_at '
  'histórico em participações reservadas ou não reservadas.';
